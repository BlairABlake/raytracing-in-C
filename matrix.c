...
matrix_t* matrix_add(matrix_t a, matrix_t b, matrix_t* buf) {
    for(int i=0; i<16; i++) {
        (*buf)[i] = a[i] + b[i];
    }
    return buf;
...
matrix_t* matrix_mul(matrix_t a, matrix_t b, matrix_t* buf) {
    matrix_t tmp = {
...
matrix_t* matrix_T(matrix_t m, matrix_t* buf) {
    matrix_t tmp;
    for(int i=0; i<16;i++) {
        tmp[4*(i%4) + i/4] = m[i];
    }
...
matrix_t* matrix_inv(matrix_t m, matrix_t* buf) {
    double inv[16], det;
